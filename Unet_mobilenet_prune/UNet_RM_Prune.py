...
# RM
def RM_layer(net, prune_scale_rate, onnx_path, logger, base_flops):
    ...
    for i, block in enumerate(net.model):
        ...
        if isinstance(block, ConvBNReLU):
            ...
            thr = get_thr(block)
            block_prue, in_mask, block_params = InvertedResidualNoRes_prune(block, in_mask, thr, concat_num, block_out_mask)
            features.append(block_prue)
    ...
    print('prune_rate: ', saved_flops/base_flops)
...
if __name__ == "__main__":
    checkpoint = '/media/byd/A264AC9264AC6AAD/DataSet/12_7_data/eps_1e1_cat/parking_slot/0104_114514/10.pth'
    ...
    RM_layer(copy.deepcopy(model), prune_scale_rate, onnx_path, logger, base_flops)
