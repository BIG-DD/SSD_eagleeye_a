...
from ..utils import letterbox, augment_hsv, random_perspective, cutout, letterbox_2head, random_perspective_2head
from ..utils import letterbox_detect, random_perspective_detect, random_perspective_segment
from ..core.general import xyxy2xywh
...
class AutoDriveDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Inputs:
        -idx: the index of images in self.db(database)(list)
        self.db(list) [a,b,c,...]
        a: (dictionary){'images':, 'information':}

        Returns:
        -images: transformed images, first passed the data augmentation in __getitem__ function(type:numpy), then apply self.transform
        ...
        """
        data = self.db[idx]
        img = cv2.imread(data["images"], cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION)
        ...
        r = resized_shape / max(h0, w0)  # resize images to img_size
...
#        _,seg2 = cv2.threshold(seg_label[:,:,2],1,255,cv2.THRESH_BINARY)
        ...
        return img, target, data["images"], shapes
    ...
    def collate_fn(batch):
        ...
        for i, l in enumerate(label):
            ...
            l_det[:, 0] = i  # add target images index for build_targets()
...
class AutoDriveDataset_2head(Dataset):
    """
    A general Dataset for some common function
    """
    ...
    def __getitem__(self, idx):
        ...
        (img, lane_label), ratio, pad = letterbox_2head((img, lane_label), resized_shape, auto=True,
        ...
        if self.is_train:
            ...
            (img, lane_label), labels = random_perspective_2head(
                combination=combination,
                targets=labels,
                degrees=self.cfg.DATASET.ROT_FACTOR,
                translate=self.cfg.DATASET.TRANSLATE,
...
class AutoDriveDataset_detect(Dataset):
    """
    A general Dataset for some common function
    """
    ...
    def __getitem__(self, idx):
        ...
        (img), ratio, pad = letterbox_detect((img), resized_shape, auto=True, scaleup=self.is_train)
        shapes = (h0, w0), ((h / h0, w / w0), pad)  # for COCO mAP rescaling
        # ratio = (w / w0, h / h0)
        # print(resized_shape)
        ...
        if self.is_train:
            combination = (img)
            (img), labels = random_perspective_detect(
                combination=combination,
                targets=labels,
                degrees=self.cfg.DATASET.ROT_FACTOR,
                translate=self.cfg.DATASET.TRANSLATE,
        ...
        img = (self.Tensor(img)).type(torch.FloatTensor)
        # (img-mean)/std
        # img = self.transform(img)
...
class AutoDriveDataset_segment(Dataset):
    """
    A general Dataset for some common function
    """
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            combination = (img, lane_label)
            (img, lane_label) = random_perspective_segment(
                combination=combination,
                degrees=self.cfg.DATASET.ROT_FACTOR,
                translate=self.cfg.DATASET.TRANSLATE,
                scale=self.cfg.DATASET.SCALE_FACTOR,
