...
def show_seg_result(ORG_IMG_SIZE, img, result, index, epoch, save_dir=None, is_ll=False,palette=None,is_demo=False,is_gt=False):
    # img = mmcv.imread(img)
    # img = img.copy()
    # seg = result[0]
    if palette is None:
    ...
    img = cv2.resize(img, (ORG_IMG_SIZE[1], ORG_IMG_SIZE[0]), interpolation=cv2.INTER_LINEAR)
