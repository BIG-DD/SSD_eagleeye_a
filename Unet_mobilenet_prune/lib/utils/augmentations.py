...
def random_perspective_2head(combination, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    """combination of img transform"""
    # torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-10, 10))
    # targets = [cls, xyxy]
...
def random_perspective_detect(combination, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    """combination of img transform"""
    # torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-10, 10))
    # targets = [cls, xyxy]
    img = combination
...
def random_perspective_segment(combination, degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    """combination of img transform"""
    # torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-10, 10))
    img, line = combination
    height = img.shape[0] + border[0] * 2  # shape(h,w,c)
...
def letterbox(combination, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
    """Resize the input images and automatically padding to suitable shape :https://zhuanlan.zhihu.com/p/172121380"""
...
def letterbox_2head(combination, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
    """Resize the input images and automatically padding to suitable shape :https://zhuanlan.zhihu.com/p/172121380"""
    # Resize images to a 32-pixel-multiple rectangle https://github.com/ultralytics/yolov3/issues/232
    img, line = combination
    shape = img.shape[:2]  # current shape [height, width]
...
def letterbox_detect(combination, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
    """Resize the input images and automatically padding to suitable shape :https://zhuanlan.zhihu.com/p/172121380"""
    # Resize images to a 32-pixel-multiple rectangle https://github.com/ultralytics/yolov3/issues/232
    img = combination
    shape = img.shape[:2]  # current shape [height, width]
