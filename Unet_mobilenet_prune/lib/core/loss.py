...
class MultiHeadLoss_2head(nn.Module):
    """
    collect all the loss we need
    """
...
class MultiHeadLoss_detect(nn.Module):
    """
    collect all the loss we need
    """
    ...
    def _forward_impl(self, predictions, targets, shapes, model):
        ...
        device = targets[0].device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        tcls, tbox, indices, anchors = build_targets(cfg, predictions, targets[0], model)  # targets
        ...
        balance = [4.0, 1.0] if no == 2 else [4.0, 1.0, 0.4]  # P3-5 or P3-6
        # calculate detection loss
...
class MultiHeadLoss_segment(nn.Module):
    """
    collect all the loss we need
    """
    ...
    def _forward_impl(self, predictions, targets, shapes, model):
        ...
        lane_line_seg_predicts = predictions.view(-1)
        ...
        pad_w = int(pad_w)
        pad_h = int(pad_h)
        _, lane_line_pred = torch.max(predictions, 1)
        ...
        lseg_ll *= cfg.LOSS.LL_SEG_GAIN * self.lambdas[0]
        liou_ll *= cfg.LOSS.LL_IOU_GAIN * self.lambdas[1]

        if cfg.TRAIN.DET_ONLY or cfg.TRAIN.ENC_DET_ONLY or cfg.TRAIN.DET_ONLY:
            lseg_ll = 0 * lseg_ll
            liou_ll = 0 * liou_ll

        loss = lseg_ll + liou_ll
        # loss = lseg
        # return loss * bs, torch.cat((lbox, lobj, lcls, loss)).detach()
        return loss, (lseg_ll.item(), liou_ll.item(), loss.item())
...
def get_loss_2head(cfg, device):
    """
    get MultiHeadLoss_2head
    ...
    loss_list = [BCEcls, BCEobj, BCEseg]
    loss = MultiHeadLoss_2head(loss_list, cfg=cfg, lambdas=cfg.LOSS.MULTI_HEAD_LAMBDA)
    return loss


def get_loss_detect(cfg, device):
    """
    get MultiHeadLoss_2head
    ...
    loss = MultiHeadLoss_detect(loss_list, cfg=cfg, lambdas=cfg.LOSS.MULTI_HEAD_LAMBDA)
    return loss


def get_loss_segment(cfg, device):
    """
    get MultiHeadLoss_2head
    ...
    loss_list = [BCEseg]
    loss = MultiHeadLoss_segment(loss_list, cfg=cfg, lambdas=cfg.LOSS.MULTI_HEAD_LAMBDA)
