...
def train(cfg, train_loader, model, criterion, optimizer, scaler, epoch, num_batch, num_warmup,
    ...
    losses_det_lbox = AverageMeter()
    losses_det_lobj = AverageMeter()
    losses_det_lcls = AverageMeter()
    ...
    lbox, lobj, lcls, lseg_ll, liou_ll = 0, 0, 0, 0, 0
    for i, (input, target, paths, shapes) in enumerate(train_loader):
        # sys.stdout.write('\r>>train  iter %d -> %d ' % (data_len, i+1))
        # sys.stdout.flush()
        ...
        with amp.autocast(enabled=device.type != 'cpu'):
            ...
            lbox, lobj, lcls, lseg_ll, liou_ll, _ = head_losses
            # print(head_losses)
        ...
        # total_loss.backward()
        # if cfg.TRAIN.SPARSITY_FACTOR > 0 and not cfg.MODEL.FINETUNE:
        #     update_BN(model, cfg.TRAIN.SPARSITY_FACTOR)
        ...
        if rank in [-1, 0]:
            ...
            # losses_det.update(lbox+lobj+lcls, input.size(0))

            losses_det_lbox.update(lbox, input.size(0))
            losses_det_lobj.update(lobj, input.size(0))
            losses_det_lcls.update(lcls, input.size(0))

            losses_seg.update(lseg_ll+liou_ll, input.size(0))
            ...
            if i % cfg.PRINT_FREQ == 0:
                msg = 'Epoch: [{0}][{1}/{2}]\t' \
                      ...
                      'det_lbox {losses_det_lbox.val:.5f} ({losses_det_lbox.avg:.5f})\t' \
                      'det_lobj {losses_det_lobj.val:.5f} ({losses_det_lobj.avg:.5f})\t'\
                      'det_lcls {losses_det_lcls.val:.5f} ({losses_det_lcls.avg:.5f})\t'\
                      'seg_Loss {seg_Loss.val:.5f} ({seg_Loss.avg:.5f})'.format(
                          epoch, i, len(train_loader), batch_time=batch_time,
                          speed=input.size(0)/batch_time.val,
                          data_time=data_time, loss=losses,
                    losses_det_lbox=losses_det_lbox,
                    losses_det_lobj=losses_det_lobj,
                    losses_det_lcls=losses_det_lcls,
                    seg_Loss=losses_seg)
...
def train_detect(cfg, train_loader, model, criterion, optimizer, scaler, epoch, num_batch, num_warmup, writer_dict, logger, device, rank=-1):
    """
    train for one epoch
    ...
    for i, (input, target, paths, shapes) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        if use_amp:
            # 当进入autocast上下文后，在这之后的cuda ops会把tensor的数据类型转换为半精度浮点型，从而在不损失训练精度的情况下加快运算。
            # 而不需要手动调用.half(),框架会自动完成转换。
            with amp.autocast(enabled=device.type != 'cpu'):
                outputs = model(input)
                total_loss, head_losses = criterion(outputs, target, shapes, model)
                lbox, lobj, lcls, _ = head_losses
                # print(head_losses)
            scaler.scale(total_loss).backward()
            scaler.step(optimizer)
            scaler.update()
            ...
            if cfg.TRAIN.SPARSITY_FACTOR > 0 and not cfg.MODEL.FINETUNE:
                # update_BN(model, cfg.TRAIN.SPARSITY_FACTOR)
                prune_scale_rate = 0.0
                if epoch > 20:
                    prune_scale_rate = 0.1 * epoch / 20
                else:
                    sparsity_mask(model, cfg.TRAIN.SPARSITY_FACTOR, prune_scale_rate)
            optimizer.step()
    ...
    msg = 'Epoch: [{0}]\t' \
          ...
          'det_lbox {losses_det_lbox.val:.5f} ({losses_det_lbox.avg:.5f})\t' \
          'det_lobj {losses_det_lobj.val:.5f} ({losses_det_lobj.avg:.5f})\t'\
          'det_lcls {losses_det_lcls.val:.5f} ({losses_det_lcls.avg:.5f})\t'.format(
...
def train_segment(cfg, train_loader, model, criterion, optimizer, scaler, epoch, num_batch, num_warmup,
          writer_dict, logger, device, rank=-1):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
...
def validate(epoch,config, val_loader, val_dataset, model, criterion, output_dir, tb_log_dir,
    ...
    for batch_i, (img, target, paths, shapes) in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            if config.TEST.PLOTS:
                if batch_i == 0:
                    for i in range(test_batch_size):
                        ...
                        _ = show_seg_result(config.DATASET.ORG_IMG_SIZE, img_test, da_seg_mask, i,epoch,save_dir)
                        _ = show_seg_result(config.DATASET.ORG_IMG_SIZE, img_test1, da_gt_mask, i, epoch, save_dir, is_gt=True)
                        ...
                        # plot_img_and_mask(img_test, seg_mask, i,epoch,save_dir)
                        img_ll1 = img_ll.copy()
                        _ = show_seg_result(config.DATASET.ORG_IMG_SIZE, img_ll, ll_seg_mask, i,epoch,save_dir, is_ll=True)
                        _ = show_seg_result(config.DATASET.ORG_IMG_SIZE, img_ll1, ll_gt_mask, i, epoch, save_dir, is_ll=True, is_gt=True)
        ...
        for si, pred in enumerate(output):
            labels = target[0][target[0][:, 0] == si, 1:]  # all object in one images
    ...
    msg = 'Epoch: [{0}]    test Loss({loss:.3f})\n' \
          'Driving area Segment: Acc({da_seg_acc:.3f})    IOU ({da_seg_iou:.3f})    mIOU({da_seg_miou:.3f})\n' \
          'Lane line Segment: Acc({ll_seg_acc:.3f})    IOU ({ll_seg_iou:.3f})  mIOU({ll_seg_miou:.3f})\n' \
          'Detect: P({p:.3f})  R({r:.3f})  mAP@0.5({map50:.3f})  mAP@0.5:0.95({map:.3f})\n' \
          'Time: inference({t_inf:.4f}s/frame)  nms({t_nms:.4f}s/frame)'.format(
        epoch, loss=losses.avg,
        da_seg_acc=da_segment_result[0],da_seg_iou=da_segment_result[1],da_seg_miou=da_segment_result[2],
        ll_seg_acc=ll_segment_result[0], ll_seg_iou=ll_segment_result[1], ll_seg_miou=ll_segment_result[2],
        p=detect_result[0], r=detect_result[1], map50=detect_result[2], map=detect_result[3],
        t_inf=t[0], t_nms=t[1])
    logger.info(msg)
...
def validate_2head(epoch, config, val_loader, val_dataset, model, criterion, output_dir, tb_log_dir,
                   writer_dict=None, logger=None, device='cpu', rank=-1):
    """
    validata
    ...
    # print(save_dir)
    _, imgsz = [check_img_size(x, s=max_stride) for x in config.MODEL.IMAGE_SIZE] #imgsz is multiple of max_stride
    batch_size = config.TRAIN.BATCH_SIZE_PER_GPU * 1    #  * len(cfg.GPUS)
    test_batch_size = config.TEST.BATCH_SIZE_PER_GPU * 1    #  * len(cfg.GPUS)
    training = False
    is_coco = False #is coco dataset
    save_conf=False # save auto-label confidences
    verbose=False
    ...
    seen = 0
    nc = model.nc
    confusion_matrix = ConfusionMatrix(nc=nc) #detector confusion matrix
    da_metric = SegmentationMetric(config.num_seg_class) #segment confusion matrix
    ll_metric = SegmentationMetric(2) #segment confusion matrix
...
def validate_detect(epoch, config, val_loader, model, criterion, output_dir, logger=None, device='cpu'):
    """
    validata
...
def validate_segment(epoch, config, val_loader, model, criterion, output_dir, logger=None, device='cpu'):
    """
    validata
    ...
    return ll_segment_result, losses.avg, t


def evaluator(config, val_loader, model, criterion, device='cpu'):
    max_stride = 32
    weights = None
    # print(save_dir)
    _, imgsz = [check_img_size(x, s=max_stride) for x in config.MODEL.IMAGE_SIZE] #imgsz is multiple of max_stride
    ...
    detect_result = np.asarray([mp, mr, map50, map])
    acc = fitness(np.array(detect_result).reshape(1, -1))

    return acc


def validate_detect_one(config, val_loader, model, device='cpu'):
    model.eval()
    save_hybrid = False
    for batch_i, (img, target, paths, shapes) in tqdm(enumerate(val_loader), total=len(val_loader)):
        if not config.DEBUG:
        ...
        with torch.no_grad():
            ...
            #NMS
            # target[0][:, 2:] *= torch.Tensor([width, height, width, height]).to(device)  # to pixels
            output = non_max_suppression(inf_out, conf_thres=config.TEST.NMS_CONF_THRESHOLD, iou_thres=config.TEST.NMS_IOU_THRESHOLD, labels=[])
            for si, pred in enumerate(output):
                det = pred.clone()
                for *xyxy, conf, cls in reversed(det):
                    print(str(xyxy)+':'+str(conf)+':'+str(cls))
            break
