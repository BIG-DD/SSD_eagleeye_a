...
from lib.core.general import wh_iou
...
def build_targets(cfg, predictions, targets, model):
    ...
    if is_parallel(model):
        det = model.module.model[model.module.detector_index]  # Detect() module
    else:
    ...
    for i in range(det.nl):
        ...
        if nt:
            ...
            # j = wh_iou(anchors, t[:, 4:6]) > cfg.TRAIN.IOU_THRESHOLD  # iou(3,n)=wh_iou(anchors(3,2), gwh(n,2))
            t = t[j]  # filter
...
def morphological_process(image, kernel_size=5, func_type=cv2.MORPH_CLOSE):
    ...
    if len(image.shape) == 3:
        raise ValueError('Binary segmentation result images should be a single channel images')
