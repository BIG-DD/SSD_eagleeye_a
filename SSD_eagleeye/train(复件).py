...
if __name__ == "__main__":
    ...
    input_shape = [512, 512]
    #--------------------------------------------#
    #   vgg, mobilenetv2, JacintoNetV2_lite, mobilenetv2_half，mobilenetv1
    #---------------------------------------------#
    backbone = "mobilenetv2_half"
    pretrained = False
    batch_size = 32
    num_workers = 16
    start_epoch = 0
    ...
    gen_val = DataLoader(val_dataset, shuffle=True, batch_size=batch_size, num_workers=num_workers, pin_memory=True,
                                drop_last=True, collate_fn=ssd_dataset_collate)
    if OPTIMIZER == 'adam':
        optimizer = optim.Adam(model_train.parameters(), lr=lr, betas=(0.937, 0.999))
    else:
        optimizer = optim.SGD(model_train.parameters(), lr=lr, momentum=0.9, weight_decay=5e-4, nesterov=True)
    ...
    for epoch in range(start_epoch, end_epoch):
        fit_one_epoch(model_train, model, criterion, loss_history, optimizer, epoch,
                epoch_step, epoch_step_val, gen_train, gen_val, end_epoch, Cuda,
                      num_batch=num_batch, num_warmup=num_warmup, END_EPOCH=end_epoch)
