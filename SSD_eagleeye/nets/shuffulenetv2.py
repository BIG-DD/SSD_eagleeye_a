...
class InvertedResidual_shufflenetv2(nn.Module):
    def __init__(self, inp, oup, stride, extend=False):
        ...
        branch_features_add = branch_features
        if extend:
            branch_features_add = branch_features * 2

        if self.stride > 1:
            self.branch1 = nn.Sequential(
                self.depthwise_conv(inp, inp, kernel_size=3, stride=self.stride, padding=1),
                nn.BatchNorm2d(inp),
                nn.Conv2d(inp, branch_features_add, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(branch_features_add),
        ...
        self.branch2 = nn.Sequential(
            ...
            nn.BatchNorm2d(branch_features),
            nn.Conv2d(branch_features, branch_features_add, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(branch_features_add),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, x):
        if self.stride == 1:
            if use_add:
                x1, x2 = x, x
                out = torch.add(x1, self.branch2(x2))
            else:
...
class ShuffleNetV2(nn.Module):
    def __init__(self, stages_repeats, stages_out_channels, num_classes=1000):#[4, 8, 4], [24, 48, 96, 192, 1024]
        ...
        for i, (name, repeats, output_channels) in enumerate(zip(stage_names, stages_repeats, self._stage_out_channels[1:])):
            extend = False
            if use_add:
                if i == 0:
                    seq = [InvertedResidual_shufflenetv2(input_channels, output_channels, 2, extend)]
                else:
                    extend = True
                    print(input_channels, output_channels)
                    seq = [InvertedResidual_shufflenetv2(input_channels // 2, output_channels // 2, 2, extend)]
...
if __name__ == "__main__":
    ...
    onnx_path = '/media/z590/G/Pytorch/Segmentation-PyTorch-master/workspace/shufflenet_v2_add.onnx'
    model = model.to('cpu')
