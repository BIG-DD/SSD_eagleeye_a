...
class AnchorBox():
    ...
    def call(self, layer_shape, mask=None):
        ...
        # --------------------------------- #
        #   每一个特征层对应的步长,例如图像尺寸为256，layer_width为32表示卷积最后特征图为32*32，
        #   step_x表示特征图中一个特征点代表原图像中8个像素。
        # --------------------------------- #
...
#---------------------------------------------------#
#   用于计算共享特征层的大小，也就是链接priorbox时网络输出feature的尺寸
#---------------------------------------------------#
def get_output_length(height, width, filter_sizes, padding, stride, priorbox_num=6):
    feature_heights = []
    feature_widths  = []
    ...
    return np.array(feature_heights)[-priorbox_num:], np.array(feature_widths)[-priorbox_num:]

def get_anchors(input_shape = [300,300], backbone = 'vgg'):
    # ----------------------------------------------------#
    ...
    #      anchors_size = [35, 76, 168, 261, 353, 445]
    # ----------------------------------------------------#
    ...
    if backbone == 'vgg':
        ...
        feature_heights, feature_widths = get_output_length(input_shape[0], input_shape[1], filter_sizes, padding, stride, priorbox_num=6)
    ...
    elif backbone == 'JacintoNetV2_lite':
        anchors_size = [35, 76, 168, 261, 353, 445]
        filter_sizes = [3, 3, 3, 3, 3, 3, 3, 3]
        stride       = [2, 2, 2, 2, 2, 2, 2, 2]
        padding      = [1, 1, 1, 1, 1, 1, 1, 1]
        feature_heights, feature_widths = get_output_length(input_shape[0], input_shape[1], filter_sizes, padding, stride, priorbox_num=5)
        aspect_ratios = [[1, 2, 3],    [1, 2, 3], [1, 2, 3], [1, 2, 3], [1, 2]]
    elif backbone == 'mobilenetv1_half':
        anchors_size = [35, 76, 168, 261, 353, 445]
        filter_sizes = [3, 3, 3, 3, 3, 3, 3, 3]
        stride       = [2, 2, 2, 2, 2, 2, 2, 2]
        padding      = [1, 1, 1, 1, 1, 1, 1, 1]
        ...
        feature_heights, feature_widths = get_output_length(input_shape[0], input_shape[1], filter_sizes, padding, stride, priorbox_num=5)
        aspect_ratios = [[1, 2, 3],    [1, 2, 3], [1, 2, 3], [1, 2, 3], [1, 2]]
    elif backbone == 'mobilenetv1_F_SSD':
        anchors_size = [35, 76, 168, 261, 353, 445]
        filter_sizes = [3, 3, 3, 3, 3, 3, 3, 3]
        stride       = [2, 2, 2, 2, 2, 2, 2, 2]
        padding      = [1, 1, 1, 1, 1, 1, 1, 1]
        ...
        feature_heights, feature_widths = get_output_length(input_shape[0], input_shape[1], filter_sizes, padding, stride, priorbox_num=5)
        aspect_ratios = [[1, 2, 3],    [1, 2, 3], [1, 2, 3], [1, 2, 3], [1, 2]]
    elif backbone == 'JacintoNetV2_yihang':
        anchors_size = [10, 20, 30]
        filter_sizes = [3, 3, 3, 3]
        stride       = [2, 2, 2, 2]
        padding      = [1, 1, 1, 1]
        feature_heights, feature_widths = get_output_length(input_shape[0], input_shape[1], filter_sizes, padding, stride, priorbox_num=2)
        aspect_ratios = [[1, 2], [1, 2, 3]]
    elif backbone == 'JacintoNetV2_nano':
        anchors_size = [10, 20, 30]
        filter_sizes = [3, 3, 3, 3]
        stride       = [2, 2, 2, 2]
        padding      = [1, 1, 1, 1]
        feature_heights, feature_widths = get_output_length(input_shape[0], input_shape[1], filter_sizes, padding, stride, priorbox_num=2)
        aspect_ratios = [[1], [1]]
    elif backbone == 'corner_point_mobilenetv2_4_tiny':
        anchors_size = [30, 20, 30]
        filter_sizes = [3, 3, 3, 3]
        stride       = [2, 2, 2, 2]
        padding      = [1, 1, 1, 1]
        feature_heights, feature_widths = get_output_length(input_shape[0], input_shape[1], filter_sizes, padding, stride, priorbox_num=2)
        feature_heights, feature_widths = feature_heights[::-1], feature_widths[::-1]
        aspect_ratios = [[1, 2], [1, 2]]
    elif backbone == 'corner_point_mobilenetv2_4':
        anchors_size = [20, 20, 20]
        filter_sizes = [3, 3, 3, 3]
        stride       = [2, 2, 2, 2]
        padding      = [1, 1, 1, 1]
        feature_heights, feature_widths = get_output_length(input_shape[0], input_shape[1], filter_sizes, padding, stride, priorbox_num=2)
        feature_heights, feature_widths = feature_heights[::-1], feature_widths[::-1]
        aspect_ratios = [[1, 2], [1, 2]]
    elif backbone == 'corner_point_mobilenetv2_4_advance':
        anchors_size = [20, 20, 20]
        filter_sizes = [3, 3, 3, 3]
        stride       = [2, 2, 2, 2]
        padding      = [1, 1, 1, 1]
